...
// The GraphQL instances to manage. Each entry is a base URL; the label shown in
// the UI is derived from its host. Swap this for service discovery later
// (k8s Endpoints / headless DNS) without touching the rest.
function getNodes() {
  return parseList(secretEnv.ADMIN_GRAPHQL_NODES).map((url) => {
    let label = url;
    try {
...
// Run `perNode(node)` against every configured instance, never rejecting: a
// failing instance becomes an `{ ok: false, error }` entry so one bad instance
// does not blank out the whole dashboard.
async function fanOut(perNode) {
...
export function register(app) {
  ...
  app.get('/api/admin/health', appendUser, requireAdmin, async (req, res) => {
    const results = await fanOut(async (node) => {
