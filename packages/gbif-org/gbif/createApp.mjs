...
// Builds the Express application shared by the local Node server (gbif/server.js) and the
// serverless entry used on platforms like Vercel (api/index.mjs). The httpServer creation,
// HMR wiring and `listen` are intentionally kept out of here so each entry point can own the
// transport: a long-lived Node process locally vs. a per-request serverless function.
//
// The app is mutated in place rather than created here so the caller can wrap it in an
// http.Server before Vite's HMR server is attached (dev mode needs that ordering).
export async function configureApp(app, { isProduction, viteDevServer } = {}) {
  // Load environment variables from .env files and merge them with process.env. On serverless
  // hosts there are no .env files at runtime, so process.env (set in the host dashboard) wins.
  const envFile = loadEnv('', process.cwd(), ['PUBLIC_']);
  const env = merge(envFile, process.env);

  const IS_PRODUCTION = isProduction ?? env.NODE_ENV === 'production';
