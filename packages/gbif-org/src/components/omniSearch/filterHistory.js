...
const keyOf = (f) => `${f.negated ? '!' : ''}${f.filterName}=${f.value}`;
 ...
 */
export function useFilterHistory(currentFilters) {
  const [history, setHistory] = useState(loadHistory);
  const prevRef = useRef(currentFilters);
  ...
  return history
    .filter(h => !activeKeys.has(keyOf(h)))
