...
export const WILDCARD_OPTION = {
  value: '*', label: 'has any value', meta: 'matches any non-null value', isWildcard: true,
};
...
export function getIntegerRangeSuggestions(query, cfg) {
...
let geoTimeVocabPromise = null;

function loadGeoTimeVocab() {
  if (!geoTimeVocabPromise) {
    geoTimeVocabPromise = fetch(GBIF_GRAPHQL_URL, {
      ...
      .then(({ data, errors }) => {
        ...
        const results = data?.geoTimeConceptSearch?.results ?? [];
...
function findConcept(vocab, name) {
  const q = name.trim().toLowerCase();
  return vocab.find(c =>
    c.name?.toLowerCase() === q || c.uiLabel?.toLowerCase() === q,
  );
}
...
const display = c => c.uiLabel ?? c.name;

const unknown = name => ({
  value: '__geotime_unknown', label: `Unknown period: ${name}`, meta: 'not in vocabulary', disabled: true,
});

// gbif-web's endFilterFn: a concept is a valid "to" given `from` iff
// concept.startAge <= from.endAge.
const isValidEndpointAfter = (c, fc) => (c.startAge ?? 0) <= (fc.endAge ?? 0);

export async function getGeoTimeRangeSuggestions(query) {
  const q = query.trim();
  ...
  if (q.includes(',')) {
    ...
    if (fromRaw === '*' && toRaw && toRaw !== '*') {
      ...
      return tc
        ? [{ value: q, label: `up to ${display(tc)}`, meta: ageMeta(tc) }]
        : [unknown(toRaw)];
    }
    // A,* → "from A onwards"
...
export function getStringRangeSuggestions(items, query) {
  const q = query.trim();
