import type { FilterFieldConfig, Preset, Suggestion } from './types';
...
export function formatRangeLabel(value?: string | null): string {
...
const encodeRangeValue = (value: string): string =>
  value
    .split(',')
...
const yearPresets = (): Preset[] => {
  const y = new Date().getFullYear();
  const span = (n: number) => `${y - (n - 1)},${y}`;
  return [
...
const booleanField = (key: string, label: string, hint: string): FilterFieldConfig => ({
  key,
  label,
  hint,
  type: 'boolean',
...
const geoTimeField = (key: string, label: string, hint: string): FilterFieldConfig => ({
  key,
  label,
  hint,
  type: 'geoTimeRange',
 ...
 */
export const FILTER_CONFIG: FilterFieldConfig[] = [
  // ── Full-text ──────────────────────────────────────────────────────────────
  {
    key: 'q',
    label: 'Text Search',
  ...
  rangeField('depth', 'Depth', 'Depth in metres below surface, or range', depthPresets),
  {
    ...
    toSuggestion: (item): Suggestion => {
      ...
      const parents = Array.isArray(item.higherRegions)
        ? [...item.higherRegions].reverse().map((p: any) => p.name).filter(Boolean)
        : [];
    ...
    resolveLabel: async (value) => {
      try {
        ...
        const parents = Array.isArray(d.higherRegions)
          ? [...d.higherRegions].reverse().map((p: any) => p.name).filter(Boolean)
          : [];
...
export const FILTER_MAP: Record<string, FilterFieldConfig> = Object.fromEntries(
  FILTER_CONFIG.map((f) => [f.key, f])
);
