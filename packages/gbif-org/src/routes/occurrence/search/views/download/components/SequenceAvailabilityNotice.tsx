...
export function SequenceAvailabilityNotice({
  ...
  return (
    <Alert variant="warning" className={`g-text-sm ${className}`}>
      <AlertTitle>
        {availability === 'none' ? (
          <FormattedMessage
      ...
      <AlertDescription>
        {availability === 'none' ? (
          ...
          <FormattedMessage
            ...
            defaultMessage="{included} of the {total} records in your search have a DNA sequence. The remaining {discarded} records will not be part of the download. Filter your search to records with DNA sequences if you want the download to match it exactly."
            values={{
              included: (
                <strong>
