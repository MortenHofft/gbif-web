...
function poolSettingSource(pool: PoolName, key: keyof PoolOverride, fallback: unknown) {
...
export function setPoolSettings(pool: PoolName, patch: PoolSettingsPatch) {
  ...
  (['concurrency', 'maxQueueDepth', 'timeoutMs', 'perRequestConcurrency'] as const).forEach(
    (key) => {
