 ...
 * pools (requestPools.ts) isolate a slow upstream; this is the global survival
 * net for when the process itself is saturated. Because it is global it sheds
 * indiscriminately, so it is meant to trigger late — after the pools have
 * already shed the actual cause.
 *
