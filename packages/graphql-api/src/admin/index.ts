 ...
 * instance still enforces authorisation independently (defence in depth) since
 * the endpoint is directly reachable on the internal network.
 *
 ...
 * allowlist (`adminUsers`) or hold one of `adminRoles`. Both default to empty,
 * so the endpoint is fail-closed until explicitly opened per environment.
 */

const adminUsers: string[] = (get(config, 'adminUsers', []) as string[]) ?? [];
const adminRoles: string[] = (get(config, 'adminRoles', []) as string[]) ?? [];
...
async function applySettings(req: Request, res: Response) {
  ...
  // Reject unknown pool names up-front so a typo does not silently no-op part of
  // a fan-out (and so the whole request is atomic-ish: validate, then apply).
  if (patch.pools) {
    const known = new Set(getPoolNames());
    const unknown = Object.keys(patch.pools).filter((p) => !known.has(p as PoolName));
    if (unknown.length) {
      res.status(400).json({
