...
const getOccurrence = async (key) => {
  const response = await fetch(`${config.apiv1}/occurrence/${key}`, { headers: { 'User-Agent': 'GBIF_graphql' } });
  if (!response.ok) {
    throw new Error(`Failed to fetch data: ${response.statusText}`);
  }
  const occurrence = await response.json();
  const { publishingOrgKey, datasetKey } = occurrence;
  const datasetResponse = await fetch(`${config.apiv1}/dataset/${datasetKey}`, { headers: { 'User-Agent': 'GBIF_graphql' } });
...
const getNode = async (endorsingNodeKey) => {
  const response = await fetch(`${config.apiv1}/node/${endorsingNodeKey}`, { headers: { 'User-Agent': 'GBIF_graphql' } });
  if (!response.ok) {
    throw new Error(`Failed to fetch data: ${response.statusText}`);
  }
