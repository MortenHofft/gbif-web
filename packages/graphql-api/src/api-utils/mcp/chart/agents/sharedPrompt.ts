...
Correct response:
{
  "graphQuery": "query Breakdown($predicate: Predicate) { occurrenceSearch(predicate: $predicate) { facet { basisOfRecord(size: 20) { key count } } } }",
  "jqQuery": "{ chart: { type: \\"pie\\" }, title: { text: \\"Breakdown by basis of record\\" }, series: [{ type: \\"pie\\", name: \\"Occurrences\\", colorByPoint: true, data: [.data.occurrenceSearch.facet.basisOfRecord[] | { name: .key, y: .count }] }] }"
}
